...
class AdminController {
    ...
    async listFolderItems(req, res, next) {
        const folderId = '11Kq9FdGy2SoOzk63055Owf9iRncvvNue'; // Use the actual folder ID from the URL
        try {
            // Set the tokens from session to the OAuth2 client
            if (req.session.tokens) {
                oauth2Client.setCredentials(req.session.tokens);
            } else {
                return res.status(401).send('Unauthorized: No tokens found in session');
            }
            ...
            console.error('Error listing files:', error); // Log the full error object for detailed information
